...
> `minimal: true` — входит в дефолтный набор базового шаблона, инстанцируется `/paf-init`. `minimal: false` — опциональные PAF-типы, клиент подключается по необходимости.
>
> **Два класса минимального набора.** PAF определяет 4 стратегических Нексуса — **объекты управления** (`market`/`customer`/`product`/`growth`) + 2 опциональных (`ops-model`, `company`). Поверх них PAF Team OS добавляет **обязательный операционный** Нексус `project-management` — **delivery map PO**: он не описывает объект управления, а фиксирует **проработку** (этапы, сроки, набор проектов/артефактов/планов) в зоне ответственности конкретного PO. Поэтому базовый шаблон `/paf-init` инстанцирует **5** дефолтных Нексусов: 4 стратегических PAF + `project-management`.
...
---
**Version:** 1.3 (добавлен обязательный операционный Нексус `project-management` — delivery map PO) · **Last updated:** 2026-06-30 · **Связанные:** [[nexus_schema]] · [[naming_conventions]] · [[GROUND/NEXUS/_registry|_registry.yaml]] · `/paf-nexus-create`
