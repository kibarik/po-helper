...
Письмо — четыре блока:
- **Цель** — 1-2 строки: кто хочет что, зачем, ключевой enabler. Мгновенный intent.
- **How to demo** — нумерованный E2E-сценарий от лица роли-приёмщика; он же ограничитель scope (что демонстрируем, за то и отвечаем). Точность «X, не Y»: дизамбигуации из встречи встраиваются в шаг («оплачен, не создан»).
- **Открытые вопросы** — сжатый список `* <ФИО> — вопрос кратко`, владелец префиксом строки; неясен → `[кому?]`.
- **Границы** — in-scope + явное «не входят в зону БФТ» + тир «Дополнительно, если возможно» (nice-to-have) + операционные заметки (сроки/эффекты).

Плюс **вложение** на согласование границ: **блок персон** (`ФИО · Роль · Департамент` — домен знаний: вес цитаты зависит от роли говорящего) + **таблица требований** (`ID · ASIS · TOBE · Связанные · Источник (цитата)`, порядок иерархии БТ → ПТ → ИТ → ФТ → НФТ). Каждая строка ← цитата участника; ASIS без реплики → `[ASIS не озвучен]`, роль/департамент неизвестны → `[не указано]`; без обогащения (детализация — на этапе глубокой проработки).
...
**Целевая модель — Haiku 4.5** (лёгкий проход дёшев и быстр). Чтобы слабая модель не срывалась в галлюцинации, команда несёт три упора у точки генерации: **скелет вывода** (копируй структуру 4 блоков + вложение), **СТОП-ГАЛЛЮЦИНАЦИИ** (негативы: департамент/роль не сказаны → `[не указано]`; никаких выдуманных технологий/алгоритмов и таймстемпов; ASIS без реплики → `[ASIS не озвучен]`; out-of-scope только в «Границы») и **самопроверку из 8 пунктов** — обязательный verify-проход перед выводом, на котором модель сама ловит и удаляет строки без цитаты. Точка расширения тиринга — конфиг-хук `models.fast_lane` ([#112](https://github.com/kibarik/po-helper/issues/112)).
