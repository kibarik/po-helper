...
Плюс **вложение** — сводная таблица требований на согласование границ: колонки `ID · ASIS · TOBE · Связанные · Источник (цитата)`, порядок иерархии БТ → ПТ → ИТ → ФТ → НФТ. Каждая строка ← цитата участника; ASIS без реплики → `[ASIS не озвучен]`; без обогащения (пороги/приоритеты/детализация — на этапе глубокой проработки).
