...
| **БФТ** | `/bft-value … /bft-deliver` (10 стадий) · быстрый вход `/bft-fast` (draft-first, письмо за 5 мин) | Бизнес-Функциональные Требования по эпику + карта внешних команд | [↓ БФТ](#-бфт--бизнес-функциональные-требования) |
...
### Быстрый вход — `/bft-fast` (draft-first)

Тяжёлый конвейер выше даёт полный документ, но идёт ~30 минут и применяет глубину вслепую наперёд. Когда 80% ясно из встречи, нужен быстрый черновик, а не полный прогон. Навык **`bft-fast`** инвертирует процесс: из Summary встречи за один лёгкий проход — **одно пересылаемое письмо в чат** за ≤5 минут.

```
/bft-fast <источник Summary> [epic_slug]
```

Письмо — три блока:
- **How to demo** — нумерованный E2E-сценарий от лица роли-приёмщика; он же ограничитель scope (что демонстрируем, за то и отвечаем).
- **Открытые вопросы** — сгруппированы по стейкхолдеру (ФИО → чекбоксы `- [ ]`); владелец неясен → `[кому?]`.
- **Границы** — in-scope + явное «не входят в зону БФТ» + операционные заметки (сроки/эффекты).

Развилка: быстрый путь заканчивается письмом. **Глубокая проработка (полный сбор контекста, конвейер `bft-*` выше) — отдельным шагом PO после ответов** на вопросы письма, а не вслепую наперёд. Гейт нулевых галлюцинаций: факт без источника в Summary → вопрос, не утверждение. Детально — [bft-fast/SKILL.md](.claude/skills/bft-fast/SKILL.md).
