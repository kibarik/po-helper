...
## 9. Модели (models) — опц., точка расширения тиринга (#112)

Конфигурация LLM-моделей для многоуровневого роутинга навыков. В настоящий момент per-command выбор модели недоступен в Claude Code; ключи зарезервированы для поддержки в будущем (#112).

```yaml
models:
  # fast_lane: модель быстрого интерактивного прохода (напр. /bft-fast, интервью).
  # Отсутствует → используется модель сессии.
  fast_lane: ""     # напр. "claude-haiku-4-5-20251001"
  # deep_lane: тяжёлая модель автономной глубокой проработки (напр. /okr-enrich).
  # Отсутствует → используется модель сессии.
