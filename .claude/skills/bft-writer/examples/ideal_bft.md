<!--
ВНИМАНИЕ: этот пример пока в СТАРОЙ 19-разд. форме и НЕ отражает актуальный lean-порядок (ЗМ-005).
Канонический порядок теперь: Оглавление → Продуктовое обоснование (ЗМ-010) → О задаче (2.1 As-Is→Gap→To-Be / 2.2 Контекст обращения / 2.3 Стейкхолдеры)
→ План демо для стейкхолдера → Критерии успеха (4.0 scope-exclusions / 4.1–4.5 БТ-НФТ) → Риски и зависимости → Открытые вопросы.
Источник истины по структуре — resources/bft_standards.md «Порядок разделов». Живой lean-референс — bft_documentation/GDSLV-kino-feed-exclusion.md.
Пример подлежит регенерации в lean-форму (см. TODO).

ideal_bft.md — пустой шаблон БФТ.
Плейсхолдеры <...> заменять реальными значениями или [УТОЧНИТЬ].
-->
---
source: <WIKI_PAGE_URL>
space: <SPACE_KEY>
version: <N>
synced: <YYYY-MM-DD>
jira: <PROJ-XXX>
status: Черновик 0.1
---

# [БФТ] <EPIC>: <Название>

Общая информация
================

| Поле | Значение |
| --- | --- |
| Название проекта | <Проект> |
| Ответственный за продукт | <ФИО> |
| Ответственный за документ | <ФИО> |
| Эпик в трекере | [<PROJECTKEY-XXX>](https://jira.mts.ru/browse/<PROJECTKEY-XXX>) |
| Статус | АНАЛИЗ / Ревью / Утверждён |

Открытые вопросы
================

<В начале документа (ЗМ-005): всё незакрытое + блокеры на видном месте. Неподтверждённое → `[УТОЧНИТЬ у {кого}]`.>

| Вопрос | Ответ | Кто ответил | Источник (встреча/почта/ТЗ) |
| --- | --- | --- | --- |
| <Вопрос> | <ответ или пусто> | <ФИО> | <источник> |

## Ключевые решения из открытых вопросов

| # | Вопрос | Решение | Кто ответил |
| --- | --- | --- | --- |
| 1 | <Вопрос> | <Решение> | <ФИО> |

Продуктовое обоснование
=======================

<Рамка всего документа (ЗМ-010): диалог с продактом в плоскости метрики, а не хотелок. Заполняется из `artefacts/value.md` (стадия /bft-value: зачем инвестировать ← KR/стратегия). Метрика верификации — единственная. Гипотеза/метрика/МДО — To-Be, якорь R2/R3 (не код). Пустой раздел (стадия /bft-value пропущена) — документ может быть принят, но это дурной тон (гейт 18 → 🟡).>

**Продуктовая гипотеза.** Если <изменение>, то <метрика> сдвинется с <база> до <цель>, потому что <причина>.

**Метрика верификации** (единственная — это KR):

| Метрика | База (до) | Цель (после) | Источник данных | MDE |
| --- | --- | --- | --- | --- |
| <одна метрика> | <текущее> | <целевое> | <где и как измеряем> | <мин. детектируемый эффект> |

**A/B-эксперимент:**

| Параметр | Значение |
| --- | --- |
| Контроль (A) | <как сейчас> |
| Вариант (B) | <что меняем> |
| Сегмент / аудитория | <на ком> |
| Сплит | <50/50> |
| Длительность / размер выборки | <срок или N> |
| Критерий успеха / стоп | <когда гипотеза подтверждена/отклонена> |

**Минимально-достаточный объём (МДО)** — что закатываем в Production, чтобы запустить A/B и верифицировать метрику (скоуп засчёта KR):

* <БТ-1 / ФТ-1> — <почему без этого метрику не верифицировать>

**Порог засчёта KR.** Релиз достаточен, когда МДО закатан в Production и метрика верифицирована A/B. На этом KR засчитан; премия команды не зависит от объёма сверх МДО.

**Дополнения сверх верификации** — не блокируют засчёт KR, докатываются по приоритету:

| Дополнение | Сверх МДО | Приоритет |
| --- | --- | --- |
| <хотелка / улучшение> | да | Высокий / Средний / Низкий |

Заинтересованные стороны
========================

| ФИО | Роль/должность | Контакты |
| --- | --- | --- |
| <ФИО> | <PO / Архитектор / …> | <email> |

Бизнес описание
===============

Данный документ сформирован для описания бизнес-функциональных, функциональных и нефункциональных требований к <краткая суть домена/функционала>.

В документе описываются требования к сценариям:
* <Сценарий 1>
* <Сценарий 2>

Границы системы
===============

<Структурой, НЕ сплошным абзацем (ЗМ-001). Каждый пункт атомарный — одна зона на буллет.>

**Входит:**
* <пункт> — <краткое описание>
* <пункт> — <краткое описание>

**Не входит:**
* <пункт> — <где реализуется / почему вне>
* <пункт> — <где реализуется / почему вне>

**Вынесено в СА / системные требования** <опц., «высота документа»>:
* <контракты / схемы БД / последовательность вызовов — что фиксирует СА, а не БФТ>

**Высота документа.** БФТ фиксирует результат и критерии приёмки для бизнеса и пользователя. Системные требования, API-контракты, схемы сообщений, вызовы между сервисами, БД-схемы и код — вне документа (зона СА / системных требований). Если нужна ссылка — указать <ссылка на СА / FNR>.

Критерии успеха и план демонстрации
===================================

<Оцифрованные критерии приёмки: метрика → базовое → цель → срок. Обязательно для фичей/миграций.
Пример: «Доля страниц с персональным FAQ: 0% → ≥X% (90 дней)»; «CR поддержки по теме FAQ: −Y%».>

| Метрика | Базовое | Цель | Срок |
| --- | --- | --- | --- |
| <метрика> | <текущее> | <цель> | <срок> |

**План демонстрации** — что, где и почему изменено с точки зрения приёмки (без технических деталей):

| Точка демо | Было (As-Is) | Стало (To-Be) | Почему изменилось | Критерий готовности к показу |
| --- | --- | --- | --- | --- |
| <экран / сценарий> | <как сейчас> | <как станет> | <бизнес-причина> | <что считаем готовым к демо> |

As-Is → Gap → To-Be
===================

<Таблицей, НЕ текстом (ЗМ-002). Пункты в ячейке через <br>; один пункт = один факт/разрыв. As-Is с якорями R1/R2; To-Be не на коде.>

| Срез | Содержание |
| --- | --- |
| **As-Is** | • <текущее поведение> <br> • <текущее поведение> |
| **Gap (проблема)** | • <разрыв> (US1) <br> • <разрыв> (US2) |
| **To-Be** | • <целевое поведение> <br> • <целевое поведение> |

Бизнес-Требования БТ
====================

| Идентификатор | Наименование | Ценность | Краткое описание | Связанные требования |
| --- | --- | --- | --- | --- |
| БТ-1 | <Название> | <Безопасность / Устранение SPOF / Сокращение TTM / …> | <Что реализуется> | [<PROJECTKEY-XXX>](https://jira.mts.ru/browse/<PROJECTKEY-XXX>) |

Пользовательские требования ПТ
==============================

| Идентификатор | Наименование | Story | Связанные требования |
| --- | --- | --- | --- |
| ПТ-1 | <Название> | **Когда** я (<актор>) <действие> **Я хочу** <результат> **Чтобы** <обоснование> | БТ-1 |

Требования к интерфейсу приёмки ИТ
==================================

Продукт: <Директ/Проводник>, Web desktop, Chrome/Yandex, 1280–1920px, адаптив.
ИТ описывает интерфейс приёмки: что видит и может сделать актор на экране и в результате. Не API-контракт (эндпоинты, протоколы, схемы запросов — зона СА).

| Идентификатор | Наименование | Требование | Связанные требования |
| --- | --- | --- | --- |
| ИТ-1 | <Экран / точка приёмки> | <Что видит актор, какие действия доступны, состояние> | ПТ-1 |
| ИТ-2 | <Доступность / адаптив> | <Платформа, разрешение, поведение при ошибке на экране> | ПТ-1 |

Функциональные требования ФТ
============================

| Идентификатор | Наименование | Приоритет | Функциональные требования | Параметры, ограничения | Связанные требования |
| --- | --- | --- | --- | --- | --- |
| ФТ-1 | <Название> | Высокий | <Поведение системы> | <TTL, идемпотентность, ограничения> | БТ-1, ПТ-1 |

Нефункциональные требования НФТ
===============================

| Идентификатор | Наименование | Описание | Связанные требования |
| --- | --- | --- | --- |
| НФТ-1 | Скорость ответа API | <≤ 2000 ms P95> | ИТ-1 |
| НФТ-2 | Идемпотентность | Повторный вызов не приводит к повторному выполнению бизнес-логики | БТ-1 |
| НФТ-3 | Мониторинг RED | Попытки/успех/неуспех, латентность, ошибки интеграций | ФТ-1 |
| НФТ-4 | Алертинг | Рост ошибок > порога, рост таймаутов, аномальный рост конфликтов | ФТ-1 |
| НФТ-5 | Логирование | <состав событий>, без чувствительных данных, хранение <N дней>, сквозной trace-id | ФТ-1 |
| НФТ-6 | Нагрузка | <100 RPS чтение / 10 RPS создание> (предварительно) | ФТ-1 |

Сценарии взаимодействия
=======================

### Акторы

| Актор | Описание |
| --- | --- |
| Покупатель | Клиент витрины |
| <Витрина / API-слой.X / Поставщик> | <Роль> |

### Сценарий приёмки (happy path + alt)

Сценарий на уровне актор → действие → результат. PlantUML — actor-level / black-box: акторы и витрина, без вызовов между внутренними сервисами и без имён сообщений/API.

```plantuml
@startuml
autonumber
actor "Пользователь" as User
participant "Витрина" as fe
activate fe
User -> fe: <действие>
fe -> fe: <состояние, видимое пользователю>
alt Ошибка — <условие>
  fe --> User: <что видит пользователь при ошибке>
end
fe --> User: <результат приёмки>
deactivate fe
@enduml
```

Атрибутивный состав сообщений (параметры и типы запросов/ответов) не входит в БФТ — зона СА / системных требований.

Зависимости
===========

| Команда | Тип зависимости | Статус согласования |
| --- | --- | --- |
| <API-слой / Шина / Процессинг / BI / 1С> | <API / данные / документация> | Подтверждено / Требует согласования |

Ограничения релиза
==================

<Рамки среды, которые релиз обязан соблюсти, иначе не состоится или навредит смежникам (из /bft-constraints). Два класса, разделять: Ф — фактологически-ограничивающее (доказанный негативный эффект, якорь); Г — гипотетически-ограничивающее (гипотеза, не доказана, проверить с владельцем). Измеримо: число / порог / дата / инвариант. Ответственный — из People Graph.>

| Идентификатор | Класс (Ф/Г) | Ограничение | Затронутый компонент | Ответственный | Эффект при нарушении | Как проверить | Статус согласования |
| --- | --- | --- | --- | --- | --- | --- | --- |
| ОГР-1 | Ф | <измеримое условие: поля события / порог RPS / окно релиза> | <компонент / сервис> | <ФИО, роль> | <доказанный негативный бизнес-эффект> | <проверка перед/на выкате> | Согласовано / Требует согласования |
| ОГР-2 | Г | <гипотеза blast-radius> | <компонент> | <ФИО или [УТОЧНИТЬ у {кого}]> | <предполагаемый эффект> | <проверить с владельцем> | Гипотеза (не проверена) |

Риски
=====

| Риск | Вероятность | Влияние | Митигация |
| --- | --- | --- | --- |
| <Риск> | Низкая/Средняя/Высокая | Низкое/Среднее/Высокое | <Митигация> |

Ревью требований
================

| Роль | Исполнитель | Статус |
| --- | --- | --- |
| Продакт | <ФИО> | <Статус> |
| Архитектор | <ФИО> | <Статус> |
| Аналитик (кросс-ревью) | <ФИО> | <Статус> |
| Разработка | <ФИО> | <Статус> |
| Тестирование | <ФИО> | <Статус> |

История изменений
=================

| # | Дата | Автор | Суть изменений |
| --- | --- | --- | --- |
| 1 | <DD.MM.YYYY> | <ФИО> | Создание страницы. Начало описания |

Дополнительные материалы
========================

| Артефакт/Ссылка | Описание |
| --- | --- |
| [Confluence <pageId>](https://confluence.mts.ru/pages/viewpage.action?pageId=<pageId>) | <ТЗ в wiki> |
| [<PROJECTKEY-XXX>](https://jira.mts.ru/browse/<PROJECTKEY-XXX>) | <Связанный БФТ/СА> |

Якоря истины
============

| Факт в БФТ | Источник (якорь) | Тип |
| --- | --- | --- |
| <факт> | [<PROJECTKEY-XXX>](https://jira.mts.ru/browse/<PROJECTKEY-XXX>) / решение PO от {дата} / ТЗ § / смежный СА | <тип> |

> Ссылки Jira/Confluence — markdown-ссылкой (ЗМ-004).

> Неподтверждённые факты помечены `[УТОЧНИТЬ у {кого}]` (без кросс-рефов) и продублированы в разделе **Открытые вопросы** в начале документа. Стиль и голос — по `resources/writing_style.md`.

<!-- ## Adversarial Review — заполняется на Стадии 6 команды /bft-gen -->
