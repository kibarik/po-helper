...
8a. **Мультихост-линзы** (`host_steps` из ≥2 шагов, напр. odi/rory-interrogation/aarrr/distribution-channels): harvest-нексус/тип задаёт **host-команда** текущего шага; блок `harvest` в реестре (если задан) — дефолт первого хоста. Команда приоритетна.
