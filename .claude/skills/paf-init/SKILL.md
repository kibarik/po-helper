...
- `sa_documentation/nexus_catalog.md` — дефолтный минимум из 4 Нексусов (§3): `customer`, `market`, `product`, `growth` (их `seed_questions`, `owner_role`, `paf_step_ref`).
...
Дефолтный минимум из `nexus_catalog.md` §3 (4 типа). Порядок — как в каталоге. Каждой записи добавь `owner` из roster (по `owner_role` каталога) и `purpose`/`name` — для читаемости; `onboarded: todo`.
...
> Если skeleton-файл `GROUND/NEXUS/_registry.yaml` уже существует (4 дефолтных без owner/purpose) — **обнови** его, дополнив поля и сохранив порядок; не дублируй записи.

### 4.3 `GROUND/NEXUS/{customer,market,product,growth}/_index.md`
...
`ttl_days` по типу (`nexus_schema.md` §2.2): `market`/`customer` = 90, `growth` = 60, `product` = 90.
...
Snapshot на момент `/paf-init`. Все 4 Нексуса созданы как placeholder'ы (CP 0.3, `onboarded: todo`).
...
- 4 дефолтных Нексуса созданы: customer, market, product, growth (CP 0.3, onboarded: todo).
- Валидатор: OK.
