...
One-shot-скилл коробки «PAF Team OS». Интервьюирует клиента → генерирует `GROUND/config.yaml` и дефолтный набор из 13 Нексусов (PAF-минимум + операционный project-management + набор po-helper intake→БФТ) из `sa_documentation/nexus_catalog.md`. Запускается один раз после `git clone`.
...
- `sa_documentation/nexus_catalog.md` — дефолтный набор из 13 Нексусов: **PAF-минимум** (§3): `customer`, `market`, `product`, `growth`; **обязательный операционный** (§3.5): `project-management` (delivery map PO); **набор po-helper intake→БФТ** (§3A): `problem`, `system-landscape`, `ownership`, `requester-domain`, `precedents`, `compliance`, `strategy`, `capacity` (их `seed_questions`, `owner_role`, `ttl_days`).
...
Дефолтный набор из `nexus_catalog.md`: PAF-минимум (§3, 4 типа) + обязательный операционный `project-management` (§3.5) + набор po-helper intake→БФТ (§3A, 8 типов) — всего 13. Порядок — как в каталоге. Каждой записи добавь `owner` из roster (по `owner_role` каталога) и `purpose`/`name` — для читаемости; `onboarded: todo`.
...
nexus_types:
  ...
  # --- Обязательный операционный Нексус PO ---
...
> Если skeleton-файл `GROUND/NEXUS/_registry.yaml` уже существует (13 дефолтных, часть без owner) — **обнови** его, дополнив `owner` из roster и сохранив порядок; не дублируй записи. `project-management` — обязательный операционный Нексус PO, всегда `source: default`. Кастомные (`source: custom`, напр. `team`) — не трогай.

### 4.3 `GROUND/NEXUS/<slug>/_index.md` (13 дефолтных)

Placeholder-нота для каждого дефолтного Нексуса (все 13 slug'ов из §4.2). Frontmatter — по `nexus_schema.md` §2 (базовый) + §2.2 (empirical онбординг-узел, low-CP). Значения:

```yaml
---
nexus: <slug>                 # любой из 13 default-slug'ов
...
<скопируй seed_questions этого типа из каталога §3 (PAF), §3.5 (project-management) или §3A (po-helper)>
...
`ttl_days` по типу: `market`/`customer`/`product`/`problem`/`system-landscape`/`requester-domain`/`strategy` = 90, `growth`/`project-management` = 60, `ownership`/`precedents`/`compliance` = 180, `capacity` = 30 (самый волатильный).

> **Готовые placeholder'ы.** Коробка уже поставляет `_index.md` для набора po-helper (`problem`, `system-landscape`, `ownership`, `requester-domain`, `precedents`, `compliance`, `strategy`, `capacity`) и для `project-management`. Если файл существует — обнови `owner` из roster и `updated`, не перегенерируй с нуля.
...
Snapshot на момент `/paf-init`. Все 13 дефолтных Нексусов созданы как placeholder'ы (CP 0.3, `onboarded: todo`).
...
- 13 дефолтных Нексусов созданы (CP 0.3, onboarded: todo):
