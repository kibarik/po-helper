...
Стендапы и командные синки (формат «кто-что-к-цели»). Не-стендап созвоны (стейкхолдер-синк, custdev) → `/summary`. Оба навыка складывают под общий корень `GROUND/PULSE`, но в разные сторы: `/daily-review` → `paths.meeting_notes` (`GROUND/PULSE/meetings`), `/summary` → `paths.summary_notes` (`GROUND/PULSE/summaries`). Любой созвон лежит в одном из двух запрашиваемых мест под `GROUND/PULSE`.
...
Каждый требующий догляда пункт → SMART-задача. Три типа с labels: `commitment` (кто-что-к-сроку), `blocker` (требует действия PO), `agreement` (PO держит договорённость). SMART-маппинг: критерий проверки → acceptance criteria, владелец → assignee, срок → в описание (у Backlog.md нет поля `due`), тело → `Срок: <дата> · ← C-NNN · {meeting_notes}/{папка}`. Reconcile: done→закрыть, slipped→сдвинуть срок в описании, new→создать (идемпотентно, без дублей). `planner.type: none` или нет MCP `backlog` → проекция и carry-forward из Backlog пропускаются, работает только слой PULSE. Правила — `resources/backlog_projection.md`.
...
- **Дополняет `/summary`** — общий корень `GROUND/PULSE`, разные сторы: стендапы через `/daily-review` (`meeting_notes`), прочие созвоны через `/summary` (`summary_notes`).
